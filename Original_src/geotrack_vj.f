C********************************************************************
C*   GEOTRACK MODEL-------THREE DIMENSIONAL TRACK ANALYSIS MODEL    *
C********************************************************************
C********************************************************************
...
C***********************************************************************
      INTEGER TITLE,OUTUNT,ON,OFF,BUFR(80),CRIBOPT

C***********************************************************************
      DATA ON/'ON  '/,OFF/'OFF '/,NPUN/0/
C***********************************************************************
C      OPEN(UNIT=2,NAME='INFILE',TYPE='OLD',FORM='FORMATTED')
C*********************************************************************
C Added Extension '.DAT' so that UNIX can open file (Viji on Aug 13.97
C*********************************************************************
      OPEN(UNIT=2,NAME='INFILE.DAT',TYPE='OLD',FORM='FORMATTED')
C      OPEN(UNIT=3,NAME='OUTFILE',TYPE='NEW',FORM='FORMATTED')
C*********************************************************************
C Added Extension '.DAT' so that UNIX can open file (Viji on Aug 13.97
C*********************************************************************
      OPEN(UNIT=3,NAME='OUTFILE.DAT',TYPE='UNKNOWN',FORM='FORMATTED')
C      OPEN(UNIT=1,NAME='SCRTH',TYPE='SCRATCH',FORM='FORMATTED')
      OPEN(UNIT=1,STATUS='SCRATCH',FORM='FORMATTED')
C*********************************************************************
C Removed Name='SCRTH'  so that UNIX can open file (Viji on Aug 13.97
C*********************************************************************
      NEWFLAG=0
      LAST=0
      IPRNT=ON
C        CHECK OF DATA IS DONE IN SCERJ AND A BRANCH MADE TO CALCUL IF
C        OK.
   10 CONTINUE
      READ(2,310,END=99) (TITLE(I),I=1,20)
  310 FORMAT(20A4)
      WRITE(3,3001)
 3001 FORMAT(1H1,/,50X,'PROGRAM GEOTRACK II',/,40X,
     1      'NONLINEAR ELASTIC MULTI-LAYER TRACK ANALYSIS',/)
      WRITE(3,300) (TITLE(I),I=1,20)
  300 FORMAT(40X,20A4)
  ...
  258 FORMAT(3I5)
      READ(2,*) (E(I),VRAT(I),HH(I),GAMMA(I),KNOT(I),KTYPE(I),KONE(I)
     1,KTWO(I),I=1,NS)
      DO 259 I=1,NS
 ...
 1050 CONTINUE
 1090 CONTINUE
      WRITE(3,312) TIEL,NSEG,TIESP,TIEWD,TA,TWT,TEI,WI,RA,RWT,REI,
     1             STIF1,NAXLES
      WRITE(3,3121) LP1,LP2,LP3,LP4,PW1,PW2,PW3,PW4,
     1             NS,LACOPT,NITER,INTERF,NOTHER,
     1            NOTENS,MINTOUT,MAXTOUT,MINSEG,MAXSEG,ITSTD,ITPRIN,
     2             ITRIAX
 ...
 3121 FORMAT(/,
  ...
  358 FORMAT(//32X,'LAYER     E(I)-K/IN.SQ.   VRAT(I)    DEPTH-IN     GA 
     1MMA-PCF  KNOT'/)
      WRITE(3,302)(I,E(I),VRAT(I),HH(I),GAMMA(I),KNOT(I),
     1I=1,NS)
  302 FORMAT(33X,I2,7X,F10.2,1X,F10.2,3X,2F10.2,1X,F10.2)
      WRITE(3,359)
  359 FORMAT(//9X,'LAYER',7X,'KTYPE',26X,'MODULUS DESCRIPTION',/)
      DO 360 I=1,NS
      IF(KTYPE(I).EQ.0)WRITE(3,3590)(I,KTYPE(I))
      IF(KTYPE(I).EQ.1)WRITE(3,3591)(I,KTYPE(I),KONE(I),KTWO(I))
      IF(KTYPE(I).EQ.2)WRITE(3,3592)(I,KTYPE(I),KONE(I),KTWO(I))
      IF(KTYPE(I).EQ.3)WRITE(3,3593)(I,KTYPE(I),KONE(I),KTWO(I))
      IF(KTYPE(I).EQ.4)WRITE(3,3594)(I,KTYPE(I))
      IF(KTYPE(I).EQ.5)WRITE(3,3595)(I,KTYPE(I))
      IF(KTYPE(I).EQ.6)WRITE(3,3596)(I,KTYPE(I))
      IF(KTYPE(I).EQ.7)WRITE(3,3597)(I,KTYPE(I))
C      IF(KTYPE(I).EQ.8)WRITE(I,KTYPE(I))
C**********************************************************
C Changed by Vijaya Rajkumar on Aug 13,97 to avoid format error
C*****************************************************************
      IF(KTYPE(I).EQ.8)WRITE(I)KTYPE(I)
  360 CONTINUE
 ...
 3593 FORMAT(10X,I2,10X,I2,10X,'E - BULK STRESS          K1= ',F8.2,
     1       5X,'K2= ',F8.2)
 3594 FORMAT(10X,I2,10X,I2,10X,'EXPERIMENTAL SHEAR STRESS REVERSAL')
 3595 FORMAT(10X,I2,10X,I2,10X,'E - DEVIATOR STRESS FOR STIFF COHESIVE
     1 SUBGRADE')
 3596 FORMAT(10X,I2,10X,I2,10X,'E - DEVIATOR STRESS FOR MEDIUM
     1 COHESIVE SUBGRADE')
 3597 FORMAT(10X,I2,10X,I2,10X,'E - DEVIATOR STRESS FOR SOFT COHESIVE
     1 SUBGRADE')
 3598 FORMAT(10X,I2,10X,I2,10X,'E - DEVIATOR STRESS FOR VERY SOFT
     1 COHESIVE SUBGRADE')  
      IF(IOPT.EQ.0) GO TO 330
      WRITE(3,314)
  314 FORMAT(//40X,'RADIUS FROM ORIGIN',5X,'IN.'/)
      WRITE(3,313)(RR(I),I=1,IR)
 ...
 2020 SIGMAV(I)=SIGMAV(I-1)+GAMMA(I-1)*(HHH(I-1)-ZZ(I))
     *+GAMMA(I)*(ZZ(I+1)-HHH(I-1))
      DO 3030 I=1,IZ1
 3030 THETAI(I)=SIGMAV(I)+KNOT(I)*SIGMAV(I)+KNOT(I)*SIGMAV(I)
C        DO THE CALCULATE ON ONE SET OF DATA AND RETURN TO READ THE NEXT
C        SET OF DATA. IF INPUT HAS BEEN CALLED BY A RETURN TO LABEL 10,
C        INPUT WILL RESUME PROCESSING AT THE LAST COLUMN AFTER THE 
C        CALCULATE COMMAND.
      NSTEPS=0
...
C***********************************************************************
      ...
      BLOCK DATA
C***********************************************************************
      INTEGER TITLE,BUFR,OUTUNT
      COMMON /SCFCCI/ E(7),VRAT(7)
      COMMON /SCFBBI/ HH(7),IR,IZ,RR(110),WGT,ZZ(20)
      COMMON /PBC/ BZ(100),PM(6,4,4)
      COMMON /FBCBI/ NOUTP
      COMMON /SCFBCI/ NS,PSI
      COMMON/CFBBBI/TITLE(20)
      COMMON /FBBBI/ II,IIZ
      COMMON /PC/ ITN,NTEST,SF
      COMMON /FRNUM/ BUFR(80),NC,ISPEC
      COMMON /FRIO/ NCARD,INUNIT,OUTUNT,IPRINT
C********************************Q**************************************
      DATA E/7*0.0/,VRAT/7*0.0/,HH/7*0.0/,IR/0/,IZ/0/,RR/110*0.0/,WGT/  
     1 0.0/,ZZ/20*0.0/,NOUTP/0/,NS/0/,PSI/0.0/,ITN/46/
      DATA TITLE/20*'    '/,II/0/,IIZ/0/,BUFR/80*'    '/,NC/0/,NCARD/0/
      DATA BZ/100*0.0/,PM/96*0.0/
      DATA INUNIT/5/,OUTUNT/6/,IPRINT/'OFF '/
C***********************************************************************
      END
      SUBROUTINE PART
C**********************************Q************************************
...
C***********************************************************************
     ...
     1 NLINE,RJ0(396),RJ1(396),TZZ,Z,COM,CMU,CSR,CST,CSZ,CTR
      ...
      COMMON/CONTA/NCNTCT(30),NUMCON
      COMMON/CFBBBI/TITLE(20)
      COMMON/SIX/MINSEG,MAXSEG,MINTOUT,GAMMA(6),KNOT(6),SIGMAV(6)
      COMMON/TWO/KONE(6),KTWO(6),NSTEPS,NITER,HHH(6),
     *THETAI(6),THETAF(6),INTERF,NP,NOTENS
      COMMON /OFF1/ OFFW(5),OFFX(5),OFFY(5),OFFZ(5),OFFXY(5),OFFXZ(5),
     *              OFFYZ(5),ANGNEW(60)
      COMMON /OFF2/ NEWFLAG,NOTHER,OFFSET
      COMMON /THREE/ TIEL,NSEG,TIESP,NTIES,NLAYER(3) 
      DIMENSION CHECKR(50)
      REAL*8 KONE,KTWO,KNOT
C***********************************************************************
...
C***********************************************************************
      ...
      NLINE=17+NS
      NPAGE=NPAGE+1
      IF(IOPT.EQ.1) WRITE(3,350)(ASTER,I=1,5),(TITLE(I),I=1,20),
     1(ASTER,I=1,4),NPAGE
  350 FORMAT (1H1//1H0,5A4,1X,20A4,1X,4A4,6H  PAGE,I2)
      IF(IOPT.EQ.1) WRITE(3,351)WGT,PSI,AR,(I,E(I),VRAT(I),HH(I)
     2,I=1,N)
...
C     COEFFICIENT MATRIX AND PRINT THE COEFFICIENT MATRIX
  ...
  511 FORMAT (5E12.5)
      ...
      DO 402 IRR=1,IR
      DO 401 ID=1,IZ
      WRITE(3,404) (IRR,ID,(COEF(IRR,ID,L),L=1,5))
  401 CONTINUE
  402 CONTINUE
...
C***********************************************************************
     ...
     1 NLINE,RJ0(396),RJ1(396),TZZ,Z,COM,CMU,CSR,CST,CSZ,CTR
      COMMON /THREE/ TIEL,NSEG,TIESP,NTIES,NLAYER(3),STA(10)
      COMMON/BLOAD/KZ,NBRT,KR,ARG,XSIDE
      COMMON /ONE/ COEF(110,6,5)
      COMMON /SCFCCI/ E(7),VRAT(7)
      COMMON/CBPC/AR,AZ(396),I1,R
      COMMON /PC/ ITN,NTEST,SF
...
C     **************
   ...
   72 CONTINUE
      ...
      IF(IOPT.EQ.1) WRITE(3,315)R,Z,CSZ,CST,CSR,CTR,BSTS
     1,COM,RDS,SST,BST
...
C
   ...
   23 TEXP=P*(H(K)-H(K+1))
      ...
      IF(TEXP.LT.ETOL) WRITE(3,210) TEXP
  210 FORMAT(1X,'TEXP=',E12.5)
   ...
   20 PM(K,M,J)=0.0
      ...
      IF(ABS(PM(K+1,1,J)).LT.TOL) WRITE(3,211) PM(K+1,1,J)
  211 FORMAT(1X,'PM(K+1,1,J)=',E12.5)
   ...
   26 CONTINUE
      ...
      IF(ABS(PM(K1,M,J)).LT.TOL) WRITE(3,201) PM(K1,M,J)
  201 FORMAT(1X,'PM(K1,M,J)=',E12.5)
   54 PM(K1,M,J)=T1*PM(K1,M,J)
      ...
      IF(ABS(PM(K1,M+2,J)).LT.TOL) WRITE(3,200) PM(K1,M+2,J)
...
C     BACKSOLVE FOR THE OTHER A,B,C,D
      ...
      IF(ABS(PM(K1,1,3)).LT.TOL) WRITE(3,202) PM(K1,1,3)
  202 FORMAT(1X,'PM(K1,1,3)=',E12.5)
   56 IF(PM(K1,1,4).EQ.0.0) GO TO 61
      ...
      IF(ABS(PM(K1,1,4)).LT.TOL) WRITE(3,203) PM(K1,1,4)
  203 FORMAT(1X,'PM(K1,1,4)=',E12.5)
   61 A(LC,K1)=(PM(K1,1,3)*C(LC,NS)+PM(K1,1,4)*D(LC,NS))/SC(K1)
      ...
      IF(ABS(PM(K1,2,3)).LT.TOL) WRITE(3,204) PM(K1,2,3)
  204 FORMAT(1X,'PM(K1,2,3)=',E12.5)
   57 IF(PM(K1,2,4).EQ.0.0) GO TO 64
      ...
      IF(ABS(PM(K1,2,4)).LT.TOL) WRITE(3,205) PM(K1,2,4)
  205 FORMAT(1X,'PM(K1,2,4)=',E12.5)
   64 B(LC,K1)=(PM(K1,2,3)*C(LC,NS)+PM(K1,2,4)*D(LC,NS))/SC(K1)
      ...
      IF(ABS(PM(K1,3,3)).LT.TOL) WRITE(3,206) PM(K1,3,3)
  206 FORMAT(1X,'PM(K1,3,3)=',E12.5)
   58 IF(PM(K1,3,4).EQ.0.0) GO TO 66
      ...
      IF(ABS(PM(K1,3,4)).LT.TOL) WRITE(3,207) PM(K1,3,4)
  207 FORMAT(1X,'PM(K1,3,4)=',E12.5)
   66 C(LC,K1)=(PM(K1,3,3)*C(LC,NS)+PM(K1,3,4)*D(LC,NS))/SC(K1)
      ...
      IF(ABS(PM(K1,4,3)).LT.TOL) WRITE(3,208) PM(K1,4,3)
  208 FORMAT(1X,'PM(K1,4,3)=',E12.5)
   59 IF(PM(K1,4,4).EQ.0.0) GO TO 91
      ...
      IF(ABS(PM(K1,4,4)).LT.TOL) WRITE(3,209) PM(K1,4,4)
...
C     WRITE(3,81)((S(I,J),J=1,30),I=1,30)
C 81  FORMAT((10E10.3))
      NA=N-1
      IF(NA.EQ.0) RETURN
...
C     WRITE(3,71)((S(I,J),J=1,30),I=1,30)
C 71  FORMAT((10E10.3))
      RETURN
      END
C******************************************************************
...
C******************************************************************
   ...
   10 CONTINUE
C     WRITE(3,31)((TEM(I,J),J=1,30),I=1,30)
C 31  FORMAT((10E11.3))
      RETURN
      END
C***********************************************************************
...
C
      WRITE(3,50)PR,PXR,PL,PXL,QR,QXR,QL,QXL,N,TS,M,MO,MI,WO,WI
      WRITE(3,52)RA,RE,RI,TA,TE,TI,ND,NE,ZM,NP,NZ
C
   83 IF(TID.EQ.TIE) M=M/2
      ...
      IF (RID.EQ.RAIL) WRITE(3,85)
   85 FORMAT(35X,'***   SYMMETRICAL IN RAIL   ***')
      IF (TID.EQ.TIE) WRITE(3,90) 
...
C
 ...
 8050 FORMAT(//,19X,
     ...
     4      5X,'SEG 5',19X,'TENSION',/)
      WRITE(3,8060) ((F(I),I=1,5),ITMARK,SUM)
 8060 FORMAT(14X,5(4X,F6.2),9X,I2,10X,F8.2)
  ...
  270 FORMAT(1X,3HTIE,5X,10HDEFLECTION,7X,8HREACTION,5X,10HDEFLECTION,
     ...
     $5HDIV 4,10X,5HDIV 5,/)
      DO 290 I=1,N
      NMOD=7-I
      WRITE(3,280) I,YI(1,NMOD),YI(2,NMOD),YI(3,NMOD),
     1             (X(NMOD,J),J=1,5)
 ...
 1004 WRITE(3,403)
  403 FORMAT(//,2X,'TIE',7X,'TIE CENTER',7X,'TIE RAIL-SEAT',32X,
     ...
     310X,'DIV 4',10X,'DIV 5',/)
      DO 4000 J=1,N
      JMOD=7-J
 4000 WRITE(3,401) J,TCM(JMOD),RSM(JMOD),(TDFL(JMOD,K),K=1,5)
 ...
 5001 TPRM=TPRM+LM(J)
  ...
  300 FORMAT(1X,/////,1X,'TIE    DEFLECTION AT INTERFACE     REACTION BE
     ...
     *IVISION 7     DIVISION 8     DIVISION 9     DIVISION 10',/)
      DO 310 I=1,N
      WRITE(3,280)I,YI(3,I),YI(4,I),(X(I,J),J=6,M)
  310 CONTINUE
  320 CONTINUE
C     **************
 ...
 7345 CONTINUE
  ...
  736 FORMAT(1H1,//,20X,'SUPERIMPOSED RESULTS FROM FINAL',
     1' ITERATION',//,20X,'----- DEFLECTIONS AND REACTIONS ----',//)
      ...
      WRITE(3,280) I,YIS(1,I),YIS(2,I),YIS(3,I),
     1            (BPS(I,J),J=1,5)
  737 CONTINUE
C
      DO 7301 I=1,MAXTIE   
 ...
 7372 WRITE(3,7373) I,TCM(I),RSM(I),LM(I),(TDFL(I,J),J=1,5)
 ...
 7374 IF(ITSTD.EQ.0) GO TO 738
  ...
  802 FORMAT(1H1,10X,'T',3X,'SEG',2X,'Z',7X,'W',8X,'THETA',
     ...
     1     RZS(ITIE,NNSEG,NDEPTH)
      IF(NDEPTH.EQ.1) WRITE(3,803)
  803 FORMAT(/,10X,'----------------------------------------',
     1'-----------------------------------------------------',/)
      WRITE(3,804) ITIE,NNSEG,NDEPTH,RWS(ITIE,NNSEG,NDEPTH),
     4            BULK,
 ...
 4413 CONTINUE
      ...
      BULK=SIG1+SIG2+SIG3
      DEV=SIG1-SIG3
      IF(NDEPTH.EQ.1) WRITE(3,447)
  447 FORMAT(/,43X,'------------------------------',
     1'--------------------',/)
      WRITE(3,446) ITIE,NNSEG,NDEPTH,RWS(ITIE,NNSEG,NDEPTH),
     1            SIG1,(DS(I),I=7,9),
     2            SIG2,(DS(I),I=4,6),
     3            SIG3,(DS(I),I=1,3),
     4            BULK,DEV
...
C***************************************************************   
 ...
 7401 FORMAT(1H1,//,43X,'SOIL VERTICAL DISPLACEMENTS AND ',
  ...
  742 FORMAT(1H1,10X,'C',3X,'SEG',2X,'Z',7X,'W',8X,'THETA',
     ...
     1     CRZS(ICRB,NNSEG,NDEPTH)
      IF(NDEPTH.EQ.1) WRITE(3,744)
  744 FORMAT(/,10X,'----------------------------------------',
     1'-----------------------------------------------------',/)
      WRITE(3,745) ICRB,NNSEG,NDEPTH,CRWS(ICRB,NNSEG,NDEPTH),
     4            BULK,
  ...
  749 CONTINUE
      ...
      BULK=SIG1+SIG2+SIG3
      DEV=SIG1-SIG3
      IF(NDEPTH.EQ.1) WRITE(3,755)
  755 FORMAT(/,43X,'------------------------------',
     1'--------------------',/)
      WRITE(3,756) ICRB,NNSEG,NDEPTH,CRWS(ICRB,NNSEG,NDEPTH),
     1            SIG1,(DS(I),I=7,9),
     2            SIG2,(DS(I),I=4,6),
     3            SIG3,(DS(I),I=1,3),
     4            BULK,DEV
 ...
 7453 FORMAT(15X,'DEPTH',8X,'W',18X,'S-XX',6X,'S-YY',6X,'S-ZZ',
     ...
     3      7X,'PSI',7X,'PSI',7X,'PSI',/)
      DO 7454 I=1,NL
      INL=I+1
      WRITE(3,7455)(ZZ(INL),OFFW(I),OFFX(I),OFFY(I),OFFZ(I),
 ...
 7402 KK=0
  ...
  739 FORMAT(1H1,//,29X,'GEOSTAT.',2X,'INIT.',2X,'INIT.',16X,
     ...
     62X,'(PSI)',/)
      ...
      IF(M.EQ.1) WRITE(3,739)
      IF(M.EQ.0) WRITE(3,729)
     ...
     1      ((SIGV(K)+G(J))*(1.+2.*KNOT(K))))/3.
      ...
      KK=K+1
      SIG(K)=SIGV(K)+G(J)
      IF(K.EQ.1) WRITE(3,751)
  751 FORMAT(/,4X,'----------------------------------------',
     1'--------------------------------------------------',/)
      WRITE(3,750) I,J,K,ZZ(KK),KNOT(K),SIG(K),PO,QO,SIGOCT,
