...
C*******************************************************
C     DATA ON/'ON  '/,OFF/'OFF '/,NPUN/0/ (THE ERROR)
C     CHARACTER REPLACED BY VALUES 0 & 1 TEMPORARILY
      DATA ON/1/,OFF/0/,NPUN/0/
...
C     REPLACING 3 WITH * TO WRITE TO CONSOLE
  ...
  300 FORMAT(40X,20A4)
     ...
     1,KTWO(I),I=1,NS)
      ...
      DO 259 I=1,NS
C***********************************************************
C CUBIC INCHES TO CUBIC FEET:::: DIVIDING BY 1728
...
C  263 FORMAT(3I5)
      ...
      NSEG=10
      NTIES=11
C CONVERTED TO INTEGER FROM REAL (CAST)
      NTIES=INT(NTIES/2.+1)
      NPOINTS=NTIES*2-1
      SEGL=TIEL/NSEG
      PSI=1.0/(TIEWD*SEGL)
 ...
 1090 CONTINUE
      ...
      WRITE(*,312) TIEL,NSEG,TIESP,TIEWD,TA,TWT,TEI,WI,RA,RWT,REI,
     1             STIF1,NAXLES
      WRITE(*,3121) LP1,LP2,LP3,LP4,PW1,PW2,PW3,PW4,
 ...
 3121 FORMAT(/,
  ...
  358 FORMAT(//32X,'LAYER     E(I)-K/IN.SQ.   VRAT(I)    DEPTH-IN     GA
     1MMA-PCF  KNOT'/)
      WRITE(*,302)(I,E(I),VRAT(I),HH(I),GAMMA(I),KNOT(I),
     1I=1,NS)
...
C LINES 215 TO 225 UNNECESSARY PARENTHESIS (COMPLEX CONSTANT) WHEN PLACED IN PARENTHESIS CONSIDERS EXPRESSION
      DO 360 I=1,NS
...
C**********************************************************************
C      DATA TITLE/20*'    '/,II/0/,IIZ/0/,BUFR/80*'    '/,NC/0/,NCARD/0/

      DATA TITLE/20*0/,II/0/,IIZ/0/,BUFR/80*0/,NC/0/,NCARD/0/


      DATA BZ/100*0.0/,PM/96*0.0/
C*******************************************************************
C   DATA INUNIT/5/,OUTUNT/6/,IPRINT/'OFF '/
      DATA INUNIT/5/,OUTUNT/6/,IPRINT/0/
C***********************************************************************
      END
      SUBROUTINE PART
C**********************************Q************************************
